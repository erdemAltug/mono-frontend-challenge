...
export const useGithubStore = defineStore({
  ...
  actions: {
    async findGithubUser() {
      const response: any = await $fetch(
        `https://api.github.com/users/${this.userName}`,
        {
          headers: {
            Authorization:
              "Bearer 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 ",
          },
        }
      );
      ...
      this.selectedLink = window.location.pathname + `/${this.generatedId}`;
    },
    ...
    async findGithubRepo() {
      ...
      console.log("repo info", this.repoData);
      var val = Math.floor(1000000 + Math.random() * 9000000);
      console.log(val);
